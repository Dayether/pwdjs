...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['form'] ?? '') === 'profile') {
    ...
    if ($min !== null && $max !== null && $min > $max) {
        $errors[] = 'Expected salary min cannot exceed max.';
    }
    if (!in_array($per, ['monthly', 'yearly', 'hourly'], true)) {
        $per = 'monthly';
    }
    if ($cur === '') {
        $cur = 'PHP';
...
$__userSkillNameSet = [];
foreach ($userSkills as $__us) {
    $__userSkillNameSet[mb_strtolower($__us['name'])] = true;
}
                    ...
                    <li class="d-flex justify-content-between gap-3">
                        <div class="flex-grow-1">
                            ...
                            <div class="small text-muted mb-1">
                                <?php $sd = htmlspecialchars($exp['start_date']);
...
<script>
    (function() {
        ...
        function syncCertDates() {
            if (certIssued && certExpiry) {
                certExpiry.min = certIssued.value || '';
            }
        }
        ...
        function calcAge() {
            if (!dobInput || !ageBadge || !dobInput.value) {
                ageBadge && (ageBadge.textContent = '');
                return;
            }
        ...
        ['dragenter', 'dragover'].forEach(ev => {
            avatarZone && avatarZone.addEventListener(ev, e => {
                e.preventDefault();
                avatarZone.classList.add('dragging');
            });
        });
        ['dragleave', 'drop'].forEach(ev => {
            avatarZone && avatarZone.addEventListener(ev, e => {
                e.preventDefault();
                avatarZone.classList.remove('dragging');
            });
        });
        ...
        function wireFileBlock(block, input, feedback) {
            ...
            ['dragenter', 'dragover'].forEach(ev => block.addEventListener(ev, e => {
                e.preventDefault();
                block.classList.add('dragging');
            }));
            ['dragleave', 'drop'].forEach(ev => block.addEventListener(ev, e => {
                e.preventDefault();
