        ...
        if ($ok) {
            ...
            if ($role === 'employer' && ($company_website !== '' || $company_phone !== '')) {
                try {
                    ...
                    $hasPhone = false;
                    foreach ($cols as $c) {
                        if (($c['Field'] ?? '') === 'company_website') $hasWebsite = true;
                        if (($c['Field'] ?? '') === 'company_phone') $hasPhone = true;
                    }
...
<style>
    ...
    .auth-page .input-floating-label label {
        ...
        /* ensure wrap in narrow columns */
        line-height: 1.15;
        /* tighter line height to fit above inputs */
        max-width: 100%;
    }
</style>
    ...
    <div class="auth-shell panels-touch">
        <div class="auth-card">
            ...
            <div class="auth-card-body">
                ...
                <?php endforeach; ?>
                <?php if (method_exists('Helpers', 'getStructuredFlashes')): $fl = Helpers::getStructuredFlashes();
                    foreach ($fl as $f): ?>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('.input-floating-label input, .input-floating-label select').forEach(inp => {
            ...
            function sync() {
                ...
                if ((inp.value || '').trim() !== '') wrap.classList.add('filled');
