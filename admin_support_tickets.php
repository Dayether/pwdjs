...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reply_ticket_id'], $_POST['reply_message'])) {
    ...
    if ($msg !== '') {
        ...
        if (!$smtpDisabled) {
            $res = Mail::send($emailTo, $ticket['name'], $subject, $body);
            if (!$res['success']) $err = $res['error'] ?? 'Send failed';
        }
    }
...
if (isset($_GET['action'], $_GET['ticket_id'])) {
    ...
    if ($a === 'delete') {
        ...
        if (isset($map[$a])) {
            SupportTicket::updateStatus($tid, $map[$a]);
            Helpers::flash('msg', 'Status updated to ' . $map[$a] . '.');
        }
...
<div class="admin-layout">
    ...
    <div class="admin-main">
        <style>
            ...
            .t-filters .search-box {
                flex: 1 1 300px;
                position: relative
            }

            .t-filters .search-box input {
                background: #101b2b;
            ...
            .t-filters button.reset-btn:hover {
                background: #213249;
                color: #fff
            }
            ...
            .st-Closed {
                background: linear-gradient(135deg, #515b67, #2a3037);
                color: #e3e9ef;
                border: 1px solid #3b444d
            }

            .ticket-actions a.action-btn {
                background: #132840;
            ...
            .ticket-actions a.action-btn:hover {
                background: #1c3a57;
                color: #fff;
                border-color: #2f5a87
            }
        ...
        <?php if (!empty($_SESSION['flash']['msg'])): ?>
            <div class="alert alert-info py-2 px-3 auto-dismiss mb-3 fade-in" style="border-left:4px solid #0ea5e9">
                <i class="bi bi-info-circle me-2"></i><?= htmlspecialchars($_SESSION['flash']['msg']);
            ...
            <div class="card border-0 shadow-sm my-4 fade-in">
                <div class="card-body p-4">
                    ...
                    <dl class="row mb-0 small">
                        ...
                        <dd class="col-sm-9"><?= nl2br(htmlspecialchars($viewTicket['subject'])); ?></dd>
                        <dt class="col-sm-3">Name</dt>
                        <dd class="col-sm-9"><?= htmlspecialchars($viewTicket['name']); ?></dd>
                        <dt class="col-sm-3">Email</dt>
                        <dd class="col-sm-9"><?= htmlspecialchars($viewTicket['email']); ?></dd>
                        <dt class="col-sm-3">User Role</dt>
                        <dd class="col-sm-9"><?= htmlspecialchars($viewTicket['user_role'] ?? '—'); ?></dd>
                        <dt class="col-sm-3">Status</dt>
                        <dd class="col-sm-9"><span class="badge text-bg-<?= ticket_status_badge($viewTicket['status']); ?>"><?= htmlspecialchars($viewTicket['status']); ?></span></dd>
                        <dt class="col-sm-3">Message</dt>
                        <dd class="col-sm-9">
                            <div class="border rounded p-2 bg-light"><?= nl2br(htmlspecialchars($viewTicket['message'])); ?></div>
                        </dd>
                        <dt class="col-sm-3">Created</dt>
                        <dd class="col-sm-9"><?= htmlspecialchars(date('M d, Y H:i', strtotime($viewTicket['created_at']))); ?></dd>
                        <?php if (!empty($viewTicket['updated_at'])): ?><dt class="col-sm-3">Updated</dt>
                            <dd class="col-sm-9"><?= htmlspecialchars(date('M d, Y H:i', strtotime($viewTicket['updated_at']))); ?></dd><?php endif; ?>
                    </dl>
                    <hr>
                    ...
                    <form method="post" class="row g-3">
                        ...
                        <div class="col-md-6"><label class="form-label">Recipient Email (override)</label><input name="reply_recipient" type="email" class="form-control" placeholder="Leave blank to use ticket email (<?= htmlspecialchars($viewTicket['email']); ?>)" value="<?= htmlspecialchars($_POST['reply_recipient'] ?? ''); ?>">
                            <div class="form-text">If left empty, original email is used.</div>
                        </div>
                        <div class="col-md-6 d-flex align-items-end">
                            <div class="small text-muted">Original: <?= htmlspecialchars($viewTicket['email']); ?></div>
                        </div>
                        <div class="col-12"><label class="form-label">Reply Message</label><textarea name="reply_message" class="form-control" rows="6" required placeholder="Type your response..."><?= htmlspecialchars($_POST['reply_message'] ?? ''); ?></textarea>
                            <div class="form-text">User receives this via email (HTML formatted).</div>
                        </div>
                        <div class="col-md-4"><label class="form-label">Update Status</label><select name="reply_new_status" class="form-select">
                                <option value="">(Leave unchanged)</option><?php foreach (['Open', 'Pending', 'Resolved', 'Closed'] as $st): ?><option value="<?= $st; ?>" <?php if (($viewTicket['status'] ?? '') === $st) echo 'disabled'; ?>><?= $st; ?></option><?php endforeach; ?>
                            </select></div>
...
<script>
    ...
    tChips?.addEventListener('click', e => {
        const c = e.target.closest('.t-chip');
        if (!c) return;
        [...tChips.querySelectorAll('.t-chip')].forEach(x => x.classList.remove('active'));
        c.classList.add('active');
        tSelect.value = c.getAttribute('data-filter') || '';
        applyTicketFilters();
    });
    tReset?.addEventListener('click', () => {
        ...
        tSelect.value = '';
        [...tChips.querySelectorAll('.t-chip')].forEach(x => x.classList.remove('active'));
        tChips.querySelector('[data-filter=""]')?.classList.add('active');
        applyTicketFilters();
    });

    function syncChip() {
        const v = tSelect.value || '';
        [...tChips.querySelectorAll('.t-chip')].forEach(ch => {
