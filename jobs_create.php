...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors && $title !== '') {
        ...
        foreach ($existing as $row) {
            ...
            $count++;
            $pct = 0;
            similar_text(mb_strtolower($title), mb_strtolower($row['title']), $pct);
            $pct = round($pct, 1);
            if ($pct >= $dupThreshold) {
                $duplicateWarning[] = [
                    'job_id' => $row['job_id'],
...
<div class="job-create-compact py-4 py-md-5">
    <div class="container">
        <style>
            ...
            .create-progress-bar[role=progressbar] {
                position: relative;
            }

            .create-progress-bar[role=progressbar] .cpb-fill {
                display: block;
                height: 100%;
            }
            ...
            .file-input-stack .plain-file {
                padding: .45rem .65rem;
            }

            /* Force select wrapper to show as filled so label does not collide with first option */
            .input-floating-label.select-filled {
                background: #fff;
            }

            .input-floating-label.select-filled>label {
                opacity: .85;
            }

            .input-floating-label.force-top {
    ...
    (function() {
        ...
        document.querySelectorAll('.skill-chip input[type=checkbox]').forEach(function(cb) {
            cb.addEventListener('change', function() {
                ...
                if (!lab) return;
                if (cb.checked) lab.classList.add('is-checked');
        ...
        (function() {
            ...
            function reqFields() {
                ...
                return Array.from(form.querySelectorAll('[required]')).filter(function(el) {
            ...
            function update() {
                ...
                if (bar) {
                    var fill = bar.querySelector('.cpb-fill');
                    if (fill) {
