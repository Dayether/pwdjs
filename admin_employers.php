...
<div class="admin-layout">
    ...
    <div class="admin-main">
        ...
        <style>
            ...
            .admin-page-header .btn.btn-primary {
                background: linear-gradient(135deg, #2d6bff, #5146ff);
            ...
            .admin-page-header .btn.btn-primary i {
                margin-right: .35rem;
            }
            ...
            .filter-bar button.reset-btn:hover {
                background: #213249;
                color: #fff
            }
            ...
            table.employers-table thead th {
                background: #142134;
                color: #ced8e6;
                font-size: .65rem;
                font-weight: 600;
            ...
            table.employers-table tbody td {
                padding: .9rem .9rem;
                vertical-align: top;
                font-size: .78rem;
                color: #d2dbe7;
            ...
            table.employers-table tbody tr:last-child td {
                border-bottom: none
            }

            table.employers-table tbody tr {
                transition: .2s;
                background: linear-gradient(90deg, rgba(255, 255, 255, 0) 0%, rgba(255, 255, 255, .015) 100%)
            }

            table.employers-table tbody tr:hover {
            ...
            @media (max-width:880px) {
                ...
                table.employers-table,
                table.employers-table thead,
                table.employers-table tbody,
                table.employers-table th,
                table.employers-table td,
                table.employers-table tr {
                    display: block
                }
                ...
                table.employers-table tbody td.actions {
                    margin-top: .4rem
                }
            }
        ...
        <div class="table-wrapper fade-in" id="tableWrapper">
            <table class="employers-table" id="employersTable">
                ...
                <tbody>
                    ...
                    <?php if (!$displayRows): ?>
                        <tr>
                            <td colspan="6" class="empty-state"><?php echo $hasStatusFilter ? 'No employers match that status.' : 'No employers found.'; ?></td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
<script>
    ...
    resetBtn?.addEventListener('click', () => {
        ...
        chips.querySelector('[data-filter=""]')?.classList.add('active');
        applyFilters();
    });
    ...
    function syncChipFromSelect() {
        ...
        [...chips.querySelectorAll('.chip')].forEach(ch => {
            ch.classList.toggle('active', (ch.getAttribute('data-filter') || '') === val);
        });
    }
    ...
    document.querySelectorAll('#employersTable thead th[data-sort]').forEach(th => {
        ...
        th.addEventListener('click', () => {
            ...
            arr.sort((a, b) => {
                ...
                if (key === 'status') {
                    return (a.getAttribute('data-status') || '').localeCompare(b.getAttribute('data-status') || '') * factor;
                }
                if (key === 'company') {
                    return (a.querySelector('.company-cell .main')?.innerText.toLowerCase() || '').localeCompare(b.querySelector('.company-cell .main')?.innerText.toLowerCase() || '') * factor;
                }
                if (key === 'business_email') {
                    return (a.children[1]?.innerText.toLowerCase() || '').localeCompare(b.children[1]?.innerText.toLowerCase() || '') * factor;
                }
                if (key === 'phone') {
                    return (a.children[2]?.innerText.toLowerCase() || '').localeCompare(b.children[2]?.innerText.toLowerCase() || '') * factor;
                }
                if (key === 'permit') {
                    return (a.children[3]?.innerText.toLowerCase() || '').localeCompare(b.children[3]?.innerText.toLowerCase() || '') * factor;
                }
