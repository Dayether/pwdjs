...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if (isset($map[$action])) {
        ...
        foreach ($_SESSION['__status_change_fps'] as $fpKey => $ts) {
            if ($ts < time() - 7200) unset($_SESSION['__status_change_fps'][$fpKey]);
        }
...
*/
        ...
        <style>
            ...
            .emp-verif-topbar a.back-btn:hover {
                background: #22364d;
                color: #fff;
                border-color: #3b5f89
            }
                ...
                <div class="card border-0 shadow-sm">
                    ...
                    <div class="card-body small">
                        <dl class="row mb-0">
                            ...
                            <dd class="col-sm-8"><?php $v = trim((string)($emp['company_name'] ?? ''));
                                                    echo $v !== '' ? Helpers::sanitizeOutput($v) : 'Not available'; ?></dd>
                            <dt class="col-sm-4">Business Email</dt>
                            <dd class="col-sm-8"><?php $v = trim((string)($emp['business_email'] ?? ''));
                                                    echo $v !== '' ? Helpers::sanitizeOutput($v) : 'Not available'; ?></dd>
                            <dt class="col-sm-4">Permit / Reg. No.</dt>
                            <dd class="col-sm-8"><?php $v = trim((string)($emp['business_permit_number'] ?? ''));
                                                    echo $v !== '' ? Helpers::sanitizeOutput($v) : 'Not available'; ?></dd>
                            <dt class="col-sm-4">Owner / Proprietor</dt>
                            <dd class="col-sm-8"><?php $v = trim((string)($emp['company_owner_name'] ?? ''));
                                                    echo $v !== '' ? Helpers::sanitizeOutput($v) : 'Not available'; ?></dd>
                            <dt class="col-sm-4">Account Owner</dt>
                            <dd class="col-sm-8"><?php $v = trim((string)($emp['name'] ?? ''));
                                                    echo $v !== '' ? Helpers::sanitizeOutput($v) : 'Not available'; ?></dd>
                            <dt class="col-sm-4">Owner Email</dt>
                            <dd class="col-sm-8"><?php $v = trim((string)($emp['email'] ?? ''));
                                                    echo $v !== '' ? Helpers::sanitizeOutput($v) : 'Not available'; ?></dd>
                            <dt class="col-sm-4">Contact Position</dt>
                            <dd class="col-sm-8"><?php $v = trim((string)($emp['contact_person_position'] ?? ''));
                                                    echo $v !== '' ? Helpers::sanitizeOutput($v) : '—'; ?></dd>
                            <dt class="col-sm-4">Contact Phone</dt>
                            <dd class="col-sm-8"><?php $v = trim((string)($emp['contact_person_phone'] ?? ''));
                                                    echo $v !== '' ? Helpers::sanitizeOutput($v) : '—'; ?></dd>
                            <dt class="col-sm-4">Created</dt>
                            <dd class="col-sm-8"><?php $v = trim((string)($emp['created_at'] ?? ''));
                                                    echo $v !== '' ? htmlspecialchars($v) : 'Not available'; ?></dd>
                            <dt class="col-sm-4">Jobs Posted</dt>
                            <dd class="col-sm-8"><?php $v = (string)($emp['job_count'] ?? '0');
                                                    echo $v !== '' ? htmlspecialchars($v) : '0'; ?></dd>
                            <?php
                            $reasonCols = [];
                            try {
                            ...
                            <?php endif;
                            if (!empty($reasonCols['last_suspension_reason']) && ($emp['employer_status'] ?? '') === 'Suspended'): $sr = trim((string)($emp['last_suspension_reason'] ?? '')); ?>
