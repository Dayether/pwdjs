...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if (in_array($action, ['Verified', 'Rejected', 'Pending'], true)) {
        ...
        if ($action === 'Suspend') {
            $ok = User::updateJobSeekerStatus($userId, 'Suspended');
            $acctFinal = $ok ? 'Suspended' : null;
        } else {
            $ok = User::updateJobSeekerStatus($userId, 'Active');
            $acctFinal = $ok ? 'Active' : null;
        }
    ...
    if ($ok) Helpers::flash('msg', $msg . $emailInfo);
    else Helpers::flash('error', $msg ?: 'Action failed.');
...
<div class="admin-layout">
    ...
    <div class="admin-main">
        <style>
            ...
            .js-verif-topbar a.back-btn {
                --clr-border: rgba(255, 255, 255, .15);
                --clr-fg: #c4d2e4;
                display: inline-flex;
                align-items: center;
            ...
            .js-verif-topbar a.back-btn:hover {
                background: #22364d;
                color: #fff;
                border-color: #3b5f89
            }
            ...
            <div class="col-lg-7">
                <div class="card border-0 shadow-sm">
                    ...
                    <div class="card-body small">
                        <dl class="row mb-0">
                            ...
                            <dd class="col-sm-8"><?php $v = trim((string)($user['name'] ?? ''));
                                                    echo $v !== '' ? Helpers::sanitizeOutput($v) : 'Not available'; ?></dd>
                            <dt class="col-sm-4">Email</dt>
                            <dd class="col-sm-8"><?php $v = trim((string)($user['email'] ?? ''));
                                                    echo $v !== '' ? Helpers::sanitizeOutput($v) : 'Not available'; ?></dd>
                            <dt class="col-sm-4">PWD ID (Full)</dt>
                            <dd class="col-sm-8">
                                <?php
                            ...
                            <dt class="col-sm-4">PWD ID (Last 4)</dt>
                            <dd class="col-sm-8"><?php echo $last4 !== '' ? Helpers::sanitizeOutput($last4) : 'Not available'; ?></dd>
                            <dt class="col-sm-4">Disability Type</dt>
                            <dd class="col-sm-8"><?php $v = trim((string)($user['disability_type'] ?? ''));
                                                    echo $v !== '' ? Helpers::sanitizeOutput($v) : 'Not available'; ?></dd>
                            <dt class="col-sm-4">Education</dt>
                            <dd class="col-sm-8"><?php $v = trim((string)($user['education_level'] ?? ''));
                                                    echo $v !== '' ? Helpers::sanitizeOutput($v) : 'Not available'; ?></dd>
                            <dt class="col-sm-4">Created</dt>
                            <dd class="col-sm-8"><?php $v = trim((string)($user['created_at'] ?? ''));
                                                    echo $v !== '' ? htmlspecialchars($v) : 'Not available'; ?></dd>
                            <dt class="col-sm-4">Updated</dt>
                            <dd class="col-sm-8"><?php $v = trim((string)($user['updated_at'] ?? ''));
                                                    echo $v !== '' ? htmlspecialchars($v) : 'Not available'; ?></dd>
                            <?php
                            ...
                            try {
                                ...
                                foreach ($cols as $c) {
                                    $reasonCols[$c['Field']] = true;
                                }
                            } catch (Throwable $e) {
                            }
                            if (!empty($reasonCols['last_status_reason'])): $rs = trim((string)($user['last_status_reason'] ?? '')); ?>
                                <dt class="col-sm-4">Last Status Reason</dt>
                                <dd class="col-sm-8"><?php echo $rs !== '' ? nl2br(Helpers::sanitizeOutput($rs)) : '—'; ?></dd>
                            <?php endif;
                            if (!empty($reasonCols['last_suspension_reason']) && ($user['job_seeker_status'] ?? '') === 'Suspended'): $sr = trim((string)($user['last_suspension_reason'] ?? '')); ?>
                                <dt class="col-sm-4">Suspension Reason</dt>
                                <dd class="col-sm-8"><?php echo $sr !== '' ? nl2br(Helpers::sanitizeOutput($sr)) : '—'; ?></dd>
                            <?php endif; ?>
                        </dl>
                    </div>
                </div>
                ...
                <div class="card border-0 shadow-sm mt-4">
                    ...
                    <div class="card-body small">
                        ...
                        ?>
                        <dl class="row mb-0">
                            <dt class="col-sm-4">Work Setup</dt>
                            <dd class="col-sm-8"><?php echo $user['preferred_work_setup'] ? Helpers::sanitizeOutput($user['preferred_work_setup']) : '—'; ?></dd>
                            <dt class="col-sm-4">Preferred Location</dt>
                            <dd class="col-sm-8"><?php echo $user['preferred_location'] ? Helpers::sanitizeOutput($user['preferred_location']) : '—'; ?></dd>
                            <dt class="col-sm-4">Expected Salary</dt>
                            <dd class="col-sm-8"><?php echo $salaryDisp ? Helpers::sanitizeOutput($salaryDisp) : '—'; ?></dd>
                            <dt class="col-sm-4">Interests</dt>
                            <dd class="col-sm-8"><?php echo $user['interests'] ? nl2br(Helpers::sanitizeOutput($user['interests'])) : '—'; ?></dd>
                            <dt class="col-sm-4">Accessibility</dt>
                            <dd class="col-sm-8"><?php echo $user['accessibility_preferences'] ? Helpers::sanitizeOutput($user['accessibility_preferences']) : '—'; ?></dd>
                        </dl>
                    </div>
                </div>
            </div>
        ...
        (function() {
            ...
            document.querySelectorAll('[data-reason-action]').forEach(btn => {
                btn.addEventListener('click', e => {
                    e.preventDefault();
                    openReason(btn.getAttribute('data-reason-action'));
                });
            });
        })();
    </script>
