...
/* =========================================================
   ...
   Example:
     pwdjsbackup/public/login.php -> login.php
     pwdjs/public/login.php       -> login.php
     public/login.php             -> login.php
     pwdjsbackup/login.php        -> login.php
...
// Back button JS fallback to clean duplicated segments (pwdjsbackup/public/... or pwdjs/public/...)
const backBtn = document.getElementById('backBtn');
if (backBtn) {
  if (backBtn.tagName === 'A') {
    let href = backBtn.getAttribute('href') || '';
  if (/pwdjsbackup\/public\/.*pwdjsbackup\/public\//i.test(href) || /pwdjs\/public\/.*pwdjs\/public\//i.test(href)) {
      const lower = href.toLowerCase();
      let idx = lower.lastIndexOf('pwdjsbackup/public/');
      if (idx === -1) idx = lower.lastIndexOf('pwdjs/public/');
      if (idx !== -1) {
        const cut = lower.includes('pwdjsbackup/public/') ? 'pwdjsbackup/public/' : 'pwdjs/public/';
        href = href.substring(idx + cut.length);
      }
      href = href.replace(/^public\//i,'').replace(/^pwdjsbackup\//i,'').replace(/^pwdjs\//i,'');
