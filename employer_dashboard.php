...
<style>
    ...
    .em-stat {
        ...
        overflow: hidden;
        box-shadow: 0 6px 18px -8px rgba(13, 110, 253, .22), 0 3px 8px rgba(0, 0, 0, .04);
    }
    ...
    .em-stat::before {
        ...
        position: absolute;
        inset: 0;
        background: radial-gradient(circle at 15% 18%, rgba(255, 193, 7, .18), transparent 60%), radial-gradient(circle at 85% 15%, rgba(102, 16, 242, .18), transparent 60%);
        opacity: .5;
    ...
    .dash-card-stat.suspended {
        border-color: #ffc107;
    }

    .dash-card-stat.closed {
        border-color: #6c757d;
    }
    ...
    .sticky-tabs {
        position: sticky;
        top: 70px;
        /* offset below main site nav (approx 56px + spacing) */
        z-index: 900;
        background: #ffffffec;
    ...
    .sticky-tabs .nav-link.active {
        background: linear-gradient(135deg, var(--primary-blue), var(--primary-purple));
        color: #fff !important;
        box-shadow: 0 4px 12px -4px rgba(13, 110, 253, .45);
    }
    ...
    .recent-app-card {
        ...
        box-shadow: 0 8px 24px -10px rgba(13, 110, 253, .22), 0 4px 12px rgba(0, 0, 0, .05);
    }
    ...
    .jobs-table-card {
        ...
        box-shadow: 0 10px 30px -12px rgba(13, 110, 253, .25), 0 4px 16px rgba(0, 0, 0, .05);
    }

    .jobs-table-card::before {
        content: "";
        position: absolute;
        inset: 0;
        background: radial-gradient(circle at 12% 18%, rgba(255, 193, 7, .20), transparent 60%), radial-gradient(circle at 85% 15%, rgba(102, 16, 242, .20), transparent 60%);
        mix-blend-mode: overlay;
        opacity: .45;
        pointer-events: none;
    }
    ...
    .mini-heading .icon {
        ...
        color: #fff;
        font-size: .9rem;
        box-shadow: 0 4px 12px -4px rgba(13, 110, 253, .55);
    }
    ...
    .quick-cta a {
        ...
        letter-spacing: .4px;
        color: #233544;
        box-shadow: 0 6px 16px -8px rgba(13, 110, 253, .22), 0 3px 8px rgba(0, 0, 0, .05);
        transition: .3s;
        display: flex;
        align-items: center;
        gap: .5rem;
    ...
    .quick-cta a:hover {
        ...
        box-shadow: 0 12px 30px -10px rgba(13, 110, 253, .35), 0 6px 18px rgba(0, 0, 0, .07);
    }
    ...
    <div class="em-hero">
        <div class="container px-0">
            ...
            <div class="em-stats-grid fade-up fade-delay-5" role="list" aria-label="Job statistics">
                <div class="em-stat" role="listitem">
                ...
                <div class="em-stat" role="listitem">
                    ...
                    <div class="value"><?php echo $openJobs; ?></div>
                    <div class="pill" style="background:#e6f8ed; color:#10632c;">Active</div>
                </div>
                <div class="em-stat" role="listitem">
                    <div class="label">Suspended</div>
                    <div class="value"><?php echo $suspendedJobs; ?></div>
                    <div class="pill" style="background:#fff6e0; color:#7a4b00;">Hold</div>
                </div>
                <div class="em-stat" role="listitem">
                    <div class="label">Closed</div>
                    <div class="value"><?php echo $closedJobs; ?></div>
                    <div class="pill" style="background:#f1f3f5; color:#4d5963;">Done</div>
                </div>
                <div class="em-stat" role="listitem">
                    <div class="label">Apps Today</div>
                    <div class="value"><?php echo $applicationsToday; ?></div>
                    <div class="pill" style="background:#eef6ff;">New</div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="jm-table-wrapper mt-3">
            <table id="jobsTable" class="table table-sm align-middle table-hover mb-0 d-none d-md-table" aria-describedby="jobsMeta">
                ...
                <tbody>
                    <?php if (!$jobs): ?>
                        <tr>
                            <td colspan="6" class="text-muted small">You have not posted any jobs yet.</td>
                        </tr>
                    <?php else: ?>
...
<script>
    (function() {
        ...
        function applyFilter() {
            ...
            let shown = 0;
            rows.forEach(r => {
                const ok = match(r, q, st);
                r.style.display = ok ? '' : 'none';
                if (ok) shown++;
            });
            cards.forEach(c => {
                const ok = match(c, q, st);
                c.style.display = ok ? '' : 'none';
            });
            const total = rows.length || cards.length;
            if (meta) {
                meta.textContent = total ? `Showing ${shown} of ${total} job${total!==1?'s':''}` : '';
            }
        }
        if (filterInput) filterInput.addEventListener('input', () => {
            clearTimeout(filterInput._t);
            filterInput._t = setTimeout(applyFilter, 180);
        });
        ...
        document.querySelectorAll('#jobsTable tbody tr .dropdown-menu, .jm-card .dropdown-menu').forEach((menu) => {
            ...
            if (anchor) {
                ...
                if (jobIdMatch) {
                    menu.dataset.jobId = jobIdMatch[1];
                }
            }
        });
...
<style>
    ...
    .jobs-manage-card {
        ...
        box-shadow: 0 6px 26px -10px rgba(13, 110, 253, .22), 0 4px 14px rgba(0, 0, 0, .05);
    }

    .jobs-manage-card::before {
        content: "";
        position: absolute;
        inset: 0;
        background: radial-gradient(circle at 12% 18%, rgba(255, 193, 7, .18), transparent 60%), radial-gradient(circle at 85% 15%, rgba(102, 16, 242, .18), transparent 60%);
        mix-blend-mode: overlay;
        opacity: .5;
        pointer-events: none;
    }
    ...
    .jm-filters .form-control,
    .jm-filters .form-select {
        min-width: 140px;
    }
    ...
    .jm-card {
        ...
        background: #ffffff;
        box-shadow: 0 6px 20px -10px rgba(13, 110, 253, .22), 0 4px 10px rgba(0, 0, 0, .05);
        position: relative;
    }

    .jm-card:hover {
        box-shadow: 0 10px 28px -12px rgba(13, 110, 253, .3), 0 6px 18px rgba(0, 0, 0, .07);
    }

    .jm-card .jm-actions .btn {
        font-size: .65rem;
        padding: .3rem .45rem;
    }
