...
try {
    ...
    foreach ($rows as $r) {
        ...
        if (array_key_exists($label, $disMap)) {
            $disMap[$label] += $cnt;
        } else {
            $disOther += $cnt;
        }
...
$pairs = [];
foreach ($disMap as $k => $v) {
    $pairs[] = ['key' => $k, 'label' => ucwords($k), 'count' => $v];
}
...
$pieCounts = [];
foreach ($pieLabels as $lbl) {
    $pieCounts[] = (int)($disMap[strtolower($lbl)] ?? 0);
}
    ...
    <div class="admin-main">
        <style>
            ...
            .metric-card {
                ...
                background: linear-gradient(140deg, #151f34, #0e1421);
                border-radius: 14px;
            ...
            .metric-card:before {
                ...
                inset: 0;
                pointer-events: none;
                background: radial-gradient(circle at 85% 15%, rgba(255, 255, 255, .09), transparent 60%);
            }
            ...
            .metric-icon {
                ...
                font-weight: 600;
                margin-bottom: .35rem;
                background: linear-gradient(135deg, #2d3d62, #182235);
            ...
            .metric-card.employers .metric-icon {
                background: linear-gradient(135deg, #1f7748, #0d3d26);
            }

            .metric-card.seekers .metric-icon {
                background: linear-gradient(135deg, #0f4c81, #09263f);
            }

            .metric-card.jobs .metric-icon {
                background: linear-gradient(135deg, #8041d9, #3f196d);
            }

            .metric-card.pending .metric-icon {
                background: linear-gradient(135deg, #aa6b08, #4a2f01);
            }

            .metric-card.pwd .metric-icon {
                background: linear-gradient(135deg, #a32020, #4a0c0c);
            }
            ...
            .quick-actions a {
                ...
                background: linear-gradient(135deg, #18263d, #132033);
                color: #d7e4f7;
                border-radius: 10px;
                display: flex;
                align-items: center;
            ...
            .quick-actions a:hover {
                border-color: #3a63ff;
                background: linear-gradient(135deg, #1d3050, #15253d);
                color: #fff;
            ...
            .section-card {
                ...
                margin-bottom: 1.4rem;
                box-shadow: 0 4px 18px -10px rgba(0, 0, 0, .55);
            }
            ...
            .fade-in-up {
                animation: fadeInUp .6s cubic-bezier(.4, .14, .3, 1) both;
            }
        ...
        <div class="section-card fade-in-up">
            ...
            <?php if ($totalSeekers > 0): ?>
                <?php foreach ($topList as $row): if ($row['count'] <= 0) continue;
                    $pct = round(($row['count'] / $totalSeekers) * 100, 1); ?>
        ...
        <script>
            ...
            (() => {
                ...
                const data = [
                    <?php foreach ($pieCounts as $c): echo (int)$c . ',';
                    endforeach; ?>
                ];
                    ...
                    if (ctxBar) {
                        ...
                        new Chart(ctxBar, {
                            ...
                            data: {
                                ...
                                datasets: [{
                                    label: 'Applications',
                                    data: [<?php foreach ($topJobCounts as $c): echo (int)$c . ',';
                                            endforeach; ?>],
